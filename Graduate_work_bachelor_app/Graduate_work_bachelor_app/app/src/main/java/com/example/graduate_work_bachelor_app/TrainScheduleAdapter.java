...
public class TrainScheduleAdapter extends BaseAdapter {
    ...
    public TrainScheduleAdapter(Context context, List<TrainSchedule> trainSchedules) {
    ...
    public Object getItem(int position) {
        return trainSchedules.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // Заполнение данных
        routeNumber.setText(trainSchedule.getTrainRoute().getRouteNumber());
        departureDate.setText(reformatDate(trainSchedule.getDepartureFlightDate().toString()));
        arrivalDate.setText(reformatDate(trainSchedule.getArrivalFlightDate().toString()));
        departureTime.setText(trainSchedule.getDepartureTime().format(timeFormatter));
        arrivalTime.setText(trainSchedule.getArrivalTime().format(timeFormatter));
        departureCity.setText(trainSchedule.getTrainRoute().getDepartureCity());
        departureStation.setText(trainSchedule.getTrainRoute().getDepartureStation());
        arrivalCity.setText(trainSchedule.getTrainRoute().getArrivalCity());
        arrivalStation.setText(trainSchedule.getTrainRoute().getArrivalStation());
        availableSeats.setText(String.format("Вільних місць: %d", trainSchedule.getTrainRoute().getCountSeats()));
        price.setText(String.format("%.2f грн", trainSchedule.getTrainRoute().getPrice()));
        schedule.setText(String.valueOf(trainSchedule.getScheduleId()));

        // Расчет времени в пути
        long travelMinutes = java.time.Duration.between(trainSchedule.getDepartureTime(), trainSchedule.getArrivalTime()).toMinutes();
        ...
        convertView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int id = trainSchedule.getScheduleId();
                Intent intent = new Intent(context, ActivityTrainSeats.class);
                intent.putExtra("scheduleId", id);
                intent.putExtra("departure_city", trainSchedule.getTrainRoute().getDepartureCity());
                intent.putExtra("arrival_city", trainSchedule.getTrainRoute().getArrivalCity());
                intent.putExtra("date", trainSchedule.getDepartureFlightDate().toString());
                intent.putExtra("price", trainSchedule.getTrainRoute().getPrice());
